...
const AICameraCapture: React.FC<AICameraCaptureProps> = ({
  onComplete,
  onCancel,
}) => {
    ...
    >
      ...
      <div className="flex-1 relative bg-black">
        ...
        {showDebug && (
          <div className="fixed top-28 left-2 right-2 bg-black/90 text-white text-xs p-3 rounded-lg z-[99999] font-mono border border-white/20">
            ...
            <div className="space-y-1">
              ...
              <div>
                Error: <span className={error ? 'text-red-400' : 'text-green-400'}>
                  {error ? `YES - ${error}` : 'NONE ✓'}
                </span>
              </div>
              <div>Preview Showing: {showPreview ? 'YES' : 'NO'}</div>
              <div>Captured: {capturedImages.length}/{CAPTURE_STEPS.length}</div>
            </div>
        ...
        <video
          ...
          playsInline
          muted
          className={`w-full h-full object-cover ${isCameraActive && !showPreview ? 'block' : 'hidden'}`}
        ...
        {isInitializing && !error ? (
            ...
            {debugLog.length > 0 && (
              <div className="mt-4 bg-black/50 rounded-lg p-3 max-w-md w-full">
                ...
                {debugLog.map((log, idx) => (
                  <p key={idx} className="text-xs text-gray-300 font-mono mb-1">{log}</p>
                ))}
              </div>
            )}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-black z-10">
            ...
            <div className="mt-6 bg-blue-900/30 rounded-lg p-4 max-w-md">
              <p className="text-sm text-blue-200">
                ...
                • Make sure camera permissions are allowed<br/>
                • Try using HTTPS instead of HTTP<br/>
          ...
          <div className="absolute inset-0 h-full flex flex-col items-center justify-center p-6 text-center bg-black z-50 overflow-y-auto">
            ...
            <h3 className="text-white text-2xl font-bold mb-2">{capturedImages.length} Photos Captured Successfully!</h3>
          ...
          <div className="absolute inset-0 h-full flex flex-col bg-black z-[99999]" style={{
            touchAction: 'auto',
            position: 'fixed',
            top: 0,
            left: 0,
            ...
            <div className="p-6 bg-gradient-to-t from-black via-black to-transparent flex gap-4 z-[99999]" style={{ touchAction: 'auto' }}>
              <button
                type="button"
                onTouchStart={(e) => {
                  e.stopPropagation();
