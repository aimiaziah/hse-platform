...
const AICameraCapture: React.FC<AICameraCaptureProps> = ({
  ...
  // Complete capture process - submit for AI analysis
  const handleComplete = () => {
    ...
    console.log('[AI Camera] Submitting images for AI analysis:', capturedImages.length);
    console.log('[AI Camera] Image data:', capturedImages.map(img => ({
      stepId: img.stepId,
    ...
    // Use setTimeout to ensure camera cleanup completes before calling onComplete
    setTimeout(() => {
      console.log('[AI Camera] Calling onComplete callback');
  ...
  const maxPhotos = 1; // Single photo for fire extinguisher
      ...
      <div className="bg-gray-900 text-white px-6 py-4 flex items-center justify-between border-b border-gray-800">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            {capturedImages.length} of {maxPhotos} image
          </p>
        </div>
        <button
