...
async function getInspectionFromLocalStorage(
  ...
  const inspections = storage.load<Record<string, unknown>[]>(storageKey, []);
  return inspections.find((i: Record<string, unknown>) => i.id === inspectionId) || null;
}

/**
 * Export First Aid inspection to Excel
 */
async function exportFirstAid(data: Record<string, unknown>): Promise<void> {
  // Transform database format to Excel export format
  const formData = (data.form_data as Record<string, unknown>) || data;
 ...
 */
async function exportManhours(data: Record<string, unknown>): Promise<void> {
  // Transform database format to Excel export format
  const formData = (data.form_data as Record<string, unknown>) || data;
  ...
  const excelData = {
    ...
    reportMonth: formData.reportMonth || (data.inspection_date as string)?.split('-')[1] || '',
    reportYear: formData.reportYear || (data.inspection_date as string)?.split('-')[0] || '',
 ...
 */
async function exportHSEInspection(data: Record<string, unknown>): Promise<void> {
  // Transform database format to Excel export format
  const formData = (data.form_data as Record<string, unknown>) || data;
  ...
  const excelData = {
    ...
    totalObservations: ((formData.observations as unknown[]) || []).length,
 ...
 */
async function exportFireExtinguisher(data: Record<string, unknown>): Promise<void> {
  // Transform database format to Excel export format
  const formData = (data.form_data as Record<string, unknown>) || data;
 ...
 */
async function exportHSEObservation(data: Record<string, unknown>): Promise<void> {
